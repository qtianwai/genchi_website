...
### 10.1 抽卡结果页 — 关联博主 + 探店视频
- **需求**：抽中的卡展示推荐该店铺的相关博主信息，用户可点击查看探店视频（复用现有视频功能）
- **当前状态**：GachaView 的 `onViewVideos` 回调已预留，MapView 集成处为 `// TODO: 跳转视频列表`；GachaCard 模型缺少博主信息字段
- **待做**：
  - 后端 `/api/gacha/draw` 返回结果中附加关联博主信息（查 author_restaurants 表）
  - GachaCard 模型新增 `authors: [Author]?` 字段
  - GachaResultView 展示博主头像列表
  - `onViewVideos` 回调接通现有的 `get_videos_by_restaurant` → 视频列表页

### 10.2 抽卡结果页 — 其他用户收藏留言 AI 摘要
- **需求**：抽中的卡展示其他用户对该店的收藏理由 AI 摘要（若有）
- **当前状态**：后端 `/api/restaurants/{id}/reviews-summary` 已实现，但 GachaResultView 中未调用
- **待做**：
  - GachaResultView 中异步调用 `getReviewsSummary`，在推荐理由下方展示摘要
  - 无摘要时不显示（note_count == 0）
...
### 10.5 个人主页成就徽章展示
- **需求**：成就直接展示在个人主页（不只是跳转入口）
- **当前状态**：ProfileView 只有 NavigationLink 跳转到成就列表页
- **待做**：
  - ProfileView 头像下方展示最近解锁的 3 个成就徽章图标
  - 显示"已解锁 X/15"进度文本
...
### 10.8 抽卡导航功能接通
- **需求**：抽中卡片后一键导航到店铺
- **当前状态**：GachaView 的 `onNavigate` 回调已预留，MapView 集成处为 `// TODO: 跳转导航`
- **待做**：
  - 复用现有导航能力（高德/百度/Apple 地图 URL Scheme）
  - 需要从 GachaCard 的 restaurant_id 查询经纬度，或在 GachaCard 中直接携带坐标

### 10.9 卡通形象升级（AI 生成 → 专业设计）
- **需求**：MVP 用 Emoji + SF Symbol，后续替换为 AI 生成或专业设计的卡通形象
- **当前状态**：FanTuanView 使用 Emoji 🍙 + 系统图标
- **待做**：
  - AI 生成饭团形象多状态图片（默认/饿了/困了/兴奋/打伞/吃卡）
  - 替换为序列帧动画或 Lottie 动画
  - 卡片正面/背面使用定制设计素材
