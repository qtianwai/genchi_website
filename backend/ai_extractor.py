...
9. **以下情况必须返回 null，不要强行给出答案**：
   ...
   - 标题中只有食物品类描述（如"肥肠粉"、"火锅"），且评论中也没有高赞评论提到具体店铺名（注意：如果评论中有高赞评论明确提到了具体店铺名，应按第12条规则处理，不要返回 null）
...
10. **店铺名识别规则**：
   ...
   - 以下不是店铺名，不要提取：
     ...
     * 人名：如"庞师不是师"（博主名，不是店名）
     * 赞助商/品牌合作标签：话题标签中出现"华商"、"中国华商"等赞助商名称，不是探店的店铺
   - 如果只能找到食物品类描述，没有具体店铺名，返回 null
   - 话题标签中的赞助商标签特征：通常与视频内容无关，如 #逢年过节中国华商 #来成都吃华商 这类是赞助商推广标签，不是店铺名

11. **分店信息必须精确提取**：
    - 如果标题或评论中明确提到分店地址（如"人民广场店"、"红谷滩店"），必须包含在name字段中
    - 如果只识别出品牌名而缺少分店信息，置信度最多为 medium
    - 例如：标题是"上海超级巨无敌好吃的不改良重庆火锅"，评论中提到"人民广场店"，店铺名应为"最山城不改良重庆火锅（人民广场店）"

12. **评论高频店铺识别（博主未确认时的补充信号）**：
    - 如果博主点赞评论和博主回复中都没有明确的店铺确认
    - 但多个高赞评论（点赞数≥20）中反复提到同一个店铺名（如出现2次以上）
    - 且有其他网友回复表示认同（如"就是这家"、"没错"等）
    - 可以据此提升置信度或给出答案
    - 例如：某店铺在TOP10评论中出现3次，且有网友佐证，即使没有博主回复也应识别
    - 这种情况下的置信度最多为 medium（因为缺乏博主直接确认）

13. **置信度判断标准**：
   - high: 标题或博主点赞评论中明确提到完整店名，且城市信息清晰
   - medium: 标题或评论中提到店名但不够完整，或城市信息不明确，或通过评论高频识别规则确认
...
6. **以下情况必须返回 null，不要强行给出答案**：
   ...
   - 标题中只有食物品类描述（如"肥肠粉"、"火锅"），且评论/回复中也没有高赞内容提到具体店铺名（注意：如果评论或回复中有高赞内容明确提到了具体店铺名，应按第9条规则处理，不要返回 null）
   - 评论中提到的"店铺名"实际上是人名、地名或食物名
   - 无法从现有信息确定唯一店铺
7. **店铺名识别规则**：
   - 店铺名通常是专有名词，如"甘记肥肠粉"、"最山城"、"陈桥老饭店"
...
10. **置信度判断标准**：
   ...
   - medium: 评论中有店名但无博主确认，或店名不够完整，或通过评论高频识别规则确认
   - low: 只能模糊推测（直接丢弃）
